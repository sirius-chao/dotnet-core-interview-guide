...
| **WebSocket** | **SSE** | **长轮询** | **WebRTC** |
|---------------|---------|------------|------------|
| 双向通信 | 单向推送 | 单向请求 | P2P通信 |
| 低延迟 | 中等延迟 | 较高延迟 | 超低延迟 |
| 实时交互、聊天、游戏 | 通知推送、状态更新 | 兼容性要求高的环境 | 音视频通话、点对点传输 |
| 标准协议，浏览器支持好 | 简单可靠，自动重连 | 兼容性最好，实现简单 | 延迟最低，但实现复杂 |
...
| **WebSocket** | **SSE** | **长轮询** | **WebRTC** |
|---------------|---------|------------|------------|
| 双向通信 + 低延迟 | 单向推送 + 简单可靠 | 兼容性最好 + 实现简单 | P2P + 超低延迟 |
| 实时聊天、游戏、协作工具 | 通知推送、状态更新、日志流 | 兼容性要求高的环境 | 音视频通话、点对点传输 |
