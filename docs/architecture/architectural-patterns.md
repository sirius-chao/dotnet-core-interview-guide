...
| 架构模式 | 适用场景 | 优势 | 劣势 |
|----------|----------|------|------|
| **分层架构** | 简单业务、单体应用 | 简单易懂、开发效率高 | 扩展性差、技术栈耦合 |
| **微服务架构** | 复杂业务、高并发 | 高扩展性、技术栈灵活 | 复杂度高、运维成本高 |
| **事件驱动架构** | 异步处理、松耦合 | 松耦合、高扩展性 | 调试困难、一致性复杂 |
| **CQRS架构** | 读写分离、高性能 | 性能优化、扩展性好 | 复杂度高、数据一致性 |

**💡 面试加分点**：提到"我会使用架构决策记录(ADR)来记录架构决策过程"
...
- **熔断降级**：Hystrix、Polly、Resilience4j

**💡 面试加分点**：提到"我会使用服务网格(Service Mesh)来管理服务间通信"
...
**一致性解决方案**：
1. **强一致性**：使用分布式事务(2PC、3PC)
2. **最终一致性**：使用事件溯源和补偿机制
3. **因果一致性**：使用向量时钟跟踪因果关系
4. **读写一致性**：使用读写分离和缓存策略
...
**💡 面试加分点**：提到"我会使用TCC(Try-Confirm-Cancel)模式处理分布式事务"
