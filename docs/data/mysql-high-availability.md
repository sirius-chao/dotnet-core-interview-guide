...
- 线程监控："使用SHOW PROCESSLIST查看binlog dump线程状态，使用SHOW SLAVE STATUS监控I/O和SQL线程状态"
...
**复制格式类型**：
- **Statement格式**：记录SQL语句，兼容性好，但可能不安全
- **Row格式**：记录行数据变化，安全可靠，但日志量大
- **Mixed格式**：自动选择，平衡安全性和性能

**选择策略**：
| 场景 | 推荐格式 | 原因 | 注意事项 |
|------|----------|------|----------|
| **数据安全要求高** | Row | 避免函数依赖问题 | 日志量增加 |
| **性能要求高** | Statement | 日志量小 | 需要测试兼容性 |
| **混合场景** | Mixed | 自动选择最优 | 需要监控日志量 |

**💡 面试加分点**：
- 提到具体配置："设置binlog_format=ROW确保数据安全，使用binlog_row_image=FULL记录完整行数据"
- 展示优化经验："定期分析binlog大小，合理设置expire_logs_days清理过期日志"
- 线程优化："设置slave_parallel_workers提升SQL线程性能，使用slave_parallel_type=LOGICAL_CLOCK实现并行复制"
- 日志优化："配置relay_log_recovery=ON确保从库重启后能正确恢复，设置sync_relay_log控制relay log刷盘频率"
...
**故障切换策略**：
1. **自动检测**：使用监控工具检测主库状态
2. **从库提升**：选择延迟最小的从库作为新主库
3. **数据同步**：确保新主库数据完整性
4. **应用切换**：更新应用连接配置

**数据一致性检查**：
- **GTID检查**：使用SHOW SLAVE STATUS检查GTID一致性
- **数据校验**：使用pt-table-checksum等工具校验数据
- **延迟监控**：监控Seconds_Behind_Master指标

**线程状态监控**：
- **I/O线程状态**：监控Slave_IO_Running状态，检查网络连接和日志传输
- **SQL线程状态**：监控Slave_SQL_Running状态，检查SQL执行错误
- **线程错误排查**：使用SHOW SLAVE STATUS查看Last_IO_Error和Last_SQL_Error

**日志文件检查**：
- **binlog状态**：使用SHOW MASTER STATUS检查binlog文件位置和大小
- **relay log状态**：使用SHOW SLAVE STATUS检查relay log文件位置和大小
- **日志完整性**：检查relay_log_recovery设置，确保从库重启后能正确恢复

**💡 面试加分点**：
- 提到具体工具："使用pt-table-sync修复主从数据不一致，使用pt-heartbeat监控复制延迟"
- 展示运维经验："建立完整的故障切换流程，定期进行故障演练"
- 线程故障排查："使用SHOW PROCESSLIST查看线程状态，使用SHOW SLAVE STATUS\G查看详细复制状态"
- 日志故障排查："检查relay log文件完整性，使用mysqlbinlog工具解析binlog和relay log内容"
...
**哨兵架构原理**
哨兵模式通过独立的监控进程实现故障检测和自动切换，就像数据库的"守护者"：
...
**💡 面试加分点**：
- 提到具体配置："哨兵集群至少需要3个节点，设置quorum确保故障检测的可靠性"
- 展示架构思维："哨兵模式解决了传统主从复制的故障检测和自动切换问题"
...
**核心配置参数**：
- **sentinel monitor**：监控的主库信息
- **sentinel down-after-milliseconds**：主观下线时间
- **sentinel failover-timeout**：故障转移超时时间
- **sentinel parallel-syncs**：并行同步从库数量

**性能优化策略**：
1. **网络优化**：减少哨兵与MySQL的网络延迟
2. **配置优化**：合理设置超时时间和重试次数
3. **监控优化**：使用INFO命令监控哨兵状态

**💡 面试加分点**：
- 提到具体参数："设置down-after-milliseconds=5000，平衡故障检测速度和误判率"
- 展示监控能力："使用SENTINEL INFO命令监控哨兵状态，关注故障转移次数"

---

## 🏗️ MySQL集群模式核心解析

### 3.1 Group Replication深度解析

**🎯 核心问题**：Group Replication如何实现分布式一致性？
...
**💡 面试加分点**：
- 提到具体配置："设置group_replication_group_name和group_replication_local_address"
- 展示技术深度："理解Paxos协议在分布式一致性中的作用，以及MySQL如何实现多版本并发控制"

### 3.2 InnoDB Cluster架构设计

**🎯 核心问题**：InnoDB Cluster如何简化集群管理？
...
- 展示运维经验："定期备份集群配置，使用MySQL Router实现应用无感知的故障切换"
...
**性能优化策略**：
1. **网络优化**：减少主从网络延迟，使用专用网络
2. **存储优化**：使用SSD存储，优化I/O性能
3. **参数优化**：调整复制相关参数
4. **架构优化**：合理设计复制拓扑
...
- 展示性能意识："在可用性和性能之间找到平衡，根据业务特点选择合适的高可用方案"
