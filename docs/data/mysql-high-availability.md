...
- **工作原理**：主库执行完事务后立即返回给客户端，不等待从库接收确认
- **一致性级别**：最终一致性，主从数据可能存在短暂不一致
- **性能影响**：对主库性能影响最小，写入性能最高
- **适用场景**：对数据一致性要求不高的业务，如日志、报表等
- **配置参数**：默认模式，无需特殊配置

**2. 半同步复制（Semi-synchronous Replication）**：
- **工作原理**：主库执行完事务后，等待至少一个从库接收并写入relay log后才返回给客户端
- **一致性级别**：强一致性，确保主库事务至少有一个从库已接收
- **性能影响**：对主库性能有一定影响，但比全同步复制小
- **适用场景**：对数据一致性有要求但允许轻微延迟的业务，如一般业务系统
- **配置参数**：
  ```ini
  # 主库配置
...
- **工作原理**：主库执行完事务后，等待所有从库都执行完该事务后才返回给客户端
- **一致性级别**：最强一致性，主从数据完全同步
- **性能影响**：对主库性能影响最大，写入性能最低
- **适用场景**：对数据一致性要求极高的业务，如金融交易系统
- **配置参数**：通过Group Replication实现，配置相对复杂
...
- 复制模式配置："根据业务需求配置半同步复制，设置合适的超时时间避免性能影响"
- 复制状态监控："使用SHOW STATUS LIKE 'Rpl_semi_sync_%'监控半同步复制状态，关注Rpl_semi_sync_master_yes_tx指标"
...
| 复制方式 | 一致性 | 性能 | 网络要求 | 适用场景 | 配置复杂度 |
|----------|--------|------|----------|----------|------------|
| **异步复制** | 最终一致 | 最高 | 低 | 非关键业务、读写分离 | 简单 |
| **半同步复制** | 强一致 | 中等 | 中等 | 一般业务、对一致性有要求 | 中等 |
| **全同步复制** | 最强一致 | 最低 | 高 | 关键业务、金融交易 | 复杂 |
...
- **复制模式**：掌握异步、半同步、全同步三种复制模式的特点和适用场景
